...
lemma combine_assn_out_in:
  "ch \<in> chs \<Longrightarrow>
   ...
   (\<up>(v = w) \<and>\<^sub>t (combine_assn chs P Q))"
  unfolding combine_assn_def
...
lemma combine_assn_waitout_wait:
  assumes "ch \<in> chs"
    ...
    and "d2 > 0"
  shows "combine_assn chs (WaitOut\<^sub>t d a p ch e rdy @\<^sub>t P) (Wait\<^sub>t d2 p2 rdy2 @\<^sub>t Q) \<Longrightarrow>\<^sub>t 
         (\<up>(d \<ge> d2) \<and>\<^sub>t (Wait\<^sub>t d2 (\<lambda>t. ParState (EState (a,p t)) (p2 t)) (merge_rdy rdy rdy2) @\<^sub>t
                        combine_assn chs (WaitOut\<^sub>t (d - d2) a (\<lambda>t. p (t + d2)) ch e rdy @\<^sub>t P) Q))"
proof -
  have *: "(\<up> (d2 \<le> d) \<and>\<^sub>t
        Wait\<^sub>t d2 (\<lambda>t. ParState (EState (a,p t)) (p2 t)) (merge_rdy rdy rdy2) @\<^sub>t
        combine_assn chs (WaitOut\<^sub>t (d - d2) a (\<lambda>t. p (t + d2)) ch e rdy @\<^sub>t P) Q) tr"
    if "(WaitOut\<^sub>t d a p ch e rdy @\<^sub>t P) tr1"
       "(Wait\<^sub>t d2 p2 rdy2 @\<^sub>t Q) tr2"
       "combine_blocks chs tr1 tr2 tr" for tr tr1 tr2
  proof -
    from that(1)[unfolded join_assn_def]
    obtain tr11 tr12 where a: "WaitOut\<^sub>t d a p ch e rdy tr11" "P tr12" "tr1 = tr11 @ tr12"
      by auto
    from that(2)[unfolded join_assn_def]
    ...
    have d: "(\<up> (d2 \<le> d) \<and>\<^sub>t
             ...
             combine_assn chs (WaitOut\<^sub>t (d - d2) a (\<lambda>t. p (t + d2)) ch e rdy @\<^sub>t P) Q) tr"
      if "0 < (d::real)"
         "combine_blocks chs (WaitBlk d (\<lambda>\<tau>. EState (a,p \<tau>)) rdy # OutBlock ch (e (a,p d)) # tr12)
      ...
      next
        ...
        show ?thesis
          ...
          subgoal for blks'
            ...
            subgoal by auto
            apply (subst join_assn_def)
            apply (rule exI[where x="[WaitBlk d (\<lambda>t. ParState (EState (a,p t)) (p2 t)) (merge_rdy rdy rdy2)]"])
            apply (rule exI[where x=blks'])
            apply (rule conjI)
            ...
            apply (rule conjI)
             prefer 2 subgoal by auto
            unfolding combine_assn_def
            apply (rule exI[where x="OutBlock ch (e (a,p d)) # tr12"])
            apply (rule exI[where x=tr22])
            apply (rule conjI)
             prefer 2 subgoal using b(2) by auto
            unfolding join_assn_def
            apply (rule exI[where x="[OutBlock ch (e (a,p d))]"])
            apply (rule exI[where x=tr12])
      ...
      next
        ...
        show ?thesis
          ...
          subgoal for blks'
            ...
            apply (subst join_assn_def)
            apply (rule exI[where x="[WaitBlk d2 (\<lambda>t. ParState (EState (a,p t)) (p2 t)) (merge_rdy rdy rdy2)]"])
            apply (rule exI[where x=blks'])
            apply (rule conjI)
            ...
            apply (rule conjI)
             prefer 2 subgoal by auto
            unfolding combine_assn_def
            apply (rule exI[where x="WaitBlk (d - d2) (\<lambda>t. EState (a,p (t + d2))) rdy # OutBlock ch (e (a,p d)) # tr12"])
            apply (rule exI[where x=tr22])
            apply (rule conjI)
             prefer 2 subgoal apply (rule conjI)
               apply (rule b(2)) by auto
            unfolding join_assn_def
            apply (rule exI[where x="[WaitBlk (d - d2) (\<lambda>t. EState (a,p (t + d2))) rdy, OutBlock ch (e (a,p d))]"])
            apply (rule exI[where x=tr12])
            apply (rule conjI)
            subgoal
              using wait_out_assn.intros(2)[of "d - d2" a "\<lambda>t. p (t + d2)" ch e rdy]
              by (simp add: d3)
            using a(2) by auto
          done
      qed
...
lemma combine_assn_waitout_in:
  assumes "ch \<in> chs"
    and "ch \<in> fst rdy"
  shows "combine_assn chs (WaitOut\<^sub>t d a p ch e rdy @\<^sub>t P) (In\<^sub>t s ch v @\<^sub>t Q) \<Longrightarrow>\<^sub>t 
         (\<up>(d = 0) \<and>\<^sub>t \<up>(v = e (a,p 0)) \<and>\<^sub>t
          (combine_assn chs P Q))"
proof -
  have *: "(\<up> (d = 0) \<and>\<^sub>t \<up> (v = e (a,p 0)) \<and>\<^sub>t combine_assn chs P Q) tr"
    if "(WaitOut\<^sub>t d a p ch e rdy @\<^sub>t P) tr1"
...
lemma combine_assn_waitout_emp:
  assumes "ch \<in> chs"
  shows "combine_assn chs (WaitOut\<^sub>t d a p ch e rdy @\<^sub>t P) emp\<^sub>t \<Longrightarrow>\<^sub>t false\<^sub>A"
  unfolding combine_assn_def
